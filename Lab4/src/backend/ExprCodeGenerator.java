...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitIntLiteral(IntLiteral nd) {
		...
		Value val = IntConstant.v(nd.getValue());
		return val;
	}
	...
	public Value visitStringLiteral(StringLiteral nd) {
		/* TODO: return something meaningful here */
		Value str = StringConstant.v(nd.getValue());
		return str;
	}
	...
	public Value visitBooleanLiteral(BooleanLiteral nd) {
		...
		Boolean bool = nd.getValue();
		int boolInt = bool ? 1 : 0;
		Value boolVal = IntConstant.v(boolInt);
		return boolVal;
	}
...
//		return visitBinaryExpr(nd);
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		...
		Value res = wrap(nd.accept(new Visitor<Value>() {
			@Override
		...
		}));	// pass to wrap() to let it take care of units.add()
